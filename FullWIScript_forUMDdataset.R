...
Depth.df = tibble(Depth = seq(0,36,0.5)) #This creates a data frame to make sure we have data for the bottom of larger lakes
#  First DO
#  Pivot Wide
Wide.WI.DO.Obs = WI.Filtered.Obs %>%
  ...
  right_join(Depth.df)%>%#changed this from full_join
  arrange(Depth) #put depths in order for interpolation
...
Long.Inter.WI.DO = Wide.Inter.WI.DO %>% #pivots to a longer format
  ...
  separate(ID, into = c("MonitoringLocationIdentifier", "Year", "DOY", "Max_Depth"), 
           sep = "/", remove = F)%>%
  mutate(Max_Depth = as.numeric(Max_Depth))%>% #Heidi added this-was readying max depth as character
...
WI.joined.inter = inner_join(Long.Inter.WI.Temp, Long.Inter.WI.DO, by=c('Depth','ID',
...
#write.csv(WI.DS.Thermo, "WIprofiles_0.5.csv")
